...
# 5. Split the 192 points into a long-format context DataFrame and a true horizon array
context_df = target_period.iloc[:168].reset_index()
context_df.columns = ["timestamp", "target"]
context_df["id"] = "NSW"  # Required identifier string for the DataFrame API

horizon_true = target_period.iloc[168:]

# 6. Check missing or infinite values
assert not context_df["target"].isna().any(), "Context contains NaN values!"

# 7. Save outputs
# Save the intermediate context DataFrame
